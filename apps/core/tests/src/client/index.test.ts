...
describe('client entry point', () => {
  it('exports the client factories and the stream consumer', () => {
    expect(Object.keys(client).sort()).toEqual(['consumeStream', 'createRouterClient', 'createTanstackQueryClient']);
  });
  ...
  it('re-exports them from the router-client module', () => {
    ...
    expect(client.consumeStream).toBe(routerClient.consumeStream);
  });
});
