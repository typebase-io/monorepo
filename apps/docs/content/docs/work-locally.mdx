...
<Callout type="warn">
  ...
  server) any time you change anything inside `typebase/`, whether that's an action, your schema, or your auth config. Re-running replaces
  `typebase/_server/` with a fresh build, but your `.env` and installed `node_modules` are preserved.
</Callout>
