...
<Callout type="warn">
  ...
  action, your schema, your auth config, or your env schema. Or use [watch mode](#rebuild-on-every-change) to have it re-run for you. Re-running
  replaces `typebase/_server/` with a fresh build, but your `.env` and installed `node_modules` are preserved.
</Callout>
