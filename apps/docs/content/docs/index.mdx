...
**`env.ts`**: declares the [environment variables](/docs/env) your server needs. They're validated when the server boots and typed on the handler context, so a missing variable is a startup error instead of an `undefined` in production.
