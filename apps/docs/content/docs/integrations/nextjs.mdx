---
title: Next.js
description: Set up Typebase with Next.js, covering the client, TanStack Query, Server Components, Server Actions, and authentication.
---
...
The CLI writes your server URL to `.env` under a key that depends on where the server is, so all three coexist: `TYPEBASE_APP_URL_LOCAL` for a [local run](/docs/work-locally), `TYPEBASE_APP_URL_DEV` for `dev`, and `TYPEBASE_APP_URL` for `prod`. Use these for all server-side calls (Server Components, Server Actions), with local at the front of the chain:
...
When developing locally, set `NEXT_PUBLIC_TYPEBASE_APP_URL` to whichever server your frontend should hit. Against a local run:
