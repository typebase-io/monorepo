...
import type { AuthSession } from './typebase/_generated/db';
...
The CLI writes your server URL to `.env` under a key that depends on where the server is, so all three coexist: `TYPEBASE_APP_URL_LOCAL` for a [local run](/docs/work-locally), `TYPEBASE_APP_URL_DEV` for `dev`, and `TYPEBASE_APP_URL` for `prod`. Use these for all server-side calls (load functions, form actions), with local at the front of the chain:

```ts title="src/lib/typebase/client.ts"
import { TYPEBASE_APP_URL, TYPEBASE_APP_URL_DEV, TYPEBASE_APP_URL_LOCAL } from '$env/static/private';

url: TYPEBASE_APP_URL_LOCAL || TYPEBASE_APP_URL_DEV || TYPEBASE_APP_URL;
```

<Callout type="warn">
  `$env/static/private` fails to build on a key that isn't in `.env` at all. If you haven't started a local run yet, either add an empty
  `TYPEBASE_APP_URL_LOCAL=` line or use `$env/dynamic/private` for that one.
</Callout>
...
When developing locally, set `PUBLIC_TYPEBASE_APP_URL` to whichever server your frontend should hit. Against a local run:

```bash title=".env"
PUBLIC_TYPEBASE_APP_URL=$TYPEBASE_APP_URL_LOCAL
```

Or against the dev deployment:
