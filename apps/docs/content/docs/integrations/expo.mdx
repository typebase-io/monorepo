---
title: Expo
description: Set up Typebase with Expo — TanStack Query client setup and authentication with SecureStore.
---
...
After running `npx typebase-io-cli deploy`, the CLI saves your server URL to `.env`. The key depends on the target — `TYPEBASE_APP_URL` for `prod`, `TYPEBASE_APP_URL_DEV` for `dev` — so both can coexist. Expo requires a different prefix for variables exposed to the app — add this to your `.env` file:
