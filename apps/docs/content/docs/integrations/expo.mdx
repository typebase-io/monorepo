...
The CLI writes your server URL to `.env` under a key that depends on where the server is, so all three coexist: `TYPEBASE_APP_URL_LOCAL` for a [local run](/docs/work-locally), `TYPEBASE_APP_URL_DEV` for `dev`, and `TYPEBASE_APP_URL` for `prod`. Expo requires a different prefix for variables exposed to the app, so add this to your `.env` file:

```bash title=".env"
EXPO_PUBLIC_TYPEBASE_APP_URL=$TYPEBASE_APP_URL
```

When developing locally, point the public variable at whichever server you want. Against a local run:

```bash title=".env"
EXPO_PUBLIC_TYPEBASE_APP_URL=$TYPEBASE_APP_URL_LOCAL
```
...
<Callout type="warn">
  A local run listens on `127.0.0.1`, which a physical device or an Android emulator cannot reach. Replace the host with your machine's LAN address —
  `10.0.2.2` for the Android emulator — and add that origin to `trustedOrigins` in `auth.ts`.
</Callout>
