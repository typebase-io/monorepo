---
title: Nuxt
description: Set up Typebase with Nuxt, covering the client, Tanstack Query, composables, and authentication.
---
...
<Callout type="info">
  The examples below show one common way to integrate Typebase with Nuxt. They're a starting point, not a requirement. Typebase doesn't impose any
...
When developing locally, put the local key at the front of the chain. A local run then wins whenever one is running, and falls back to the dev deployment when it isn't:

```ts title="nuxt.config.ts"
export default defineNuxtConfig({
  runtimeConfig: {
    public: {
      TYPEBASE_APP_URL: process.env.TYPEBASE_APP_URL_LOCAL || process.env.TYPEBASE_APP_URL_DEV,
    },
  },
});
```
