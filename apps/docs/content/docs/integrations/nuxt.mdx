...
After running `npx typebase-io-cli deploy`, the CLI saves your server URL to `.env` as `TYPEBASE_APP_URL`. Expose it in `nuxt.config.ts` under `runtimeConfig.public` so it's available on both server and client:
