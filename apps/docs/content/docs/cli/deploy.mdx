...
7. **Saves the URL and environment variables**: Writes the deployment URL to your project-root `.env` as `TYPEBASE_APP_URL`, along with the connection string (`DATABASE_URL` for `prod`, `DATABASE_URL_DEV` for `dev`).
...
This URL is also saved to your `.env` file:
