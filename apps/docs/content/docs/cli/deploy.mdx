...
3. **Generates server files**: Builds the server code from your actions, schema, and auth config.
...
<Callout type="info">
  ...
  The generated `src/env.ts` reads them from the Worker bindings, so `env` on the handler context works the same as on every other provider.
</Callout>
