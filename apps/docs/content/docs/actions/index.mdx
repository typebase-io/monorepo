...
```ts
export const getProfile = action.input(z.object({ id: z.number() })).handler(async ({ db, input, auth, reqHeaders }) => {
...
```ts
export const sendEmail = action.input(z.object({ to: z.email(), subject: z.string() })).handler(async ({ input }) => {
