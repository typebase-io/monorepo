...
- **Locally**: a `.env` next to the server, loaded by `dotenv`. [`start`](/docs/cli/start) fills that file from your project-root `.env` on every pass, so you set values in your project root; [`generate-server`](/docs/cli/generate-server) seeds `typebase/_server/.env` once and then leaves what's there alone. See [Work locally](/docs/work-locally#environment-variables).
