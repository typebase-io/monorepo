...
<Cards>
  ...
  <Card
    ...
    description="npm, pnpm, yarn, or bun — anything that reads a package.json. The CLI auto-detects which one your project uses."
  />
</Cards>
...
After setup, running `npx typebase-io-cli deploy dev` is a single command. The CLI saves your server URL to `.env` as `TYPEBASE_APP_URL`.
