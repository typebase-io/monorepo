...
For a handler, the output type comes from the action's `.output()` schema or is inferred from the value it returns. For a [streaming action](/docs/actions/streaming), it is an async iterator whose event type comes from `.output()` or the values the generator yields.
