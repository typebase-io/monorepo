...
The imports and the type arguments track what your project actually has. A project without auth gets `never` in the second slot and no `authConfig` import; one without an `env.ts` gets `never` in the third.
