...
- **Auth needs no extra setup.** A project with an `auth.ts` gets the local URL written into the generated server's auth config as `baseURL`, following `--port`. Do not set `BETTER_AUTH_URL` or add a `baseURL` to `typebase/auth.ts` to chase the `Base URL could not be determined` warning: a value you set wins over the one `start` writes, and a hardcoded localhost URL then ships on the next deploy.
...
Prefer `start` over `generate-server --watch --command` when the user wants to run their server locally. Prefer `generate-server` when the generated files themselves are wanted — reading, self-hosting, a non-`node` adapter, or a specific on-disk output format. Prefer `generate-server --embedded` when the server has to live inside an application the user already runs.

### Embedded servers (`generate-server --embedded`)

`generate-server` defaults to a **standalone server**: it owns a process and starts its own listener. `--embedded`, or `{ "server": { "embedded": true } }`, builds an **embedded server** instead — the same actions, database, and auth, emitted without a bootstrap for an application that already owns its process. When a user wants Typebase inside an app they already run, recommend this. Never extract handler code out of a generated entry point by hand: that copy is a fork, and every build rewrites the file it came from.

- **What it emits:** `<tb>/_handler/src/` (overridable with `--out-dir`/`server.outDir`) plus a `typebase-server.json` marker. No `index.ts`, `package.json`, `tsconfig.json`, `.env`, or package-manager config. TypeScript output uses `.js` relative import specifiers so it compiles under an ordinary app `tsconfig.json` (`nodenext` and `bundler`); `--output esm|cjs` still work.
- **`src/server.ts` exports the same three names for every adapter:** `typebaseHandler` (the mount), `router`, and `auth` when the project has auth. Only the mount's shape follows the adapter: Fastify plugin (`await app.register(typebaseHandler)`), Hono sub-app (`app.route('/', typebaseHandler)`), Connect-style Node middleware that calls `next()` on a miss (`app.use(typebaseHandler)`), and a fetch handler for Bun, Deno, and Cloudflare (`export default { fetch: typebaseHandler }`).
- **The host owns dependencies, environment, and CORS.** The first build prints an install command for the detected package manager listing only what the host `package.json` lacks, warns on version mismatches, names the env keys, states that CORS is the host's job, and prints the mount snippet; watch rebuilds stay quiet. The same data is in `typebase-server.json`, so prefer reading that file over re-running the build. Never write a nested `package.json` or install inside the output directory — a second copy of `drizzle-orm`/`better-auth`/`fastify` in one process is the failure this design avoids.
- **Nothing loads environment for it.** No `.env` is written and no dotenv import is emitted, but `src/env.ts` still validates on import, so the host must have the keys in `process.env` before it imports `server.ts`.
- **Paths:** `--actions-path` (default `/rpc`) and `--auth-path` (default `/api/auth`), also `server.actionsPath`/`server.authPath`, independent of each other and embedded-only. Passing either flag to a standalone build errors; a configured value is ignored there. The auth path is also written into the generated auth config as better-auth's `basePath`, and a `basePath` set in the user's own `<tb>/auth.ts` wins with a build warning. A custom actions path changes the client `url`: the client appends `/rpc` to a string, so pass the actions path minus its trailing `/rpc`, or pass a `URL`/resolver naming the full endpoint.
- **`--port` conflicts with `--embedded`** and is rejected when `server.embedded` is set; a configured `server.port` is ignored instead. `start` always builds a standalone server and ignores `server.embedded`.
- **Regeneration** replaces a directory only when it is empty, absent, carries `typebase-server.json`, or holds the older `@typebase-io/server` `package.json`. An embedded output directory usually sits in the user's own source tree; do not point `--out-dir` at a directory holding hand-written code.
...
- better-auth `Base URL could not be determined`: expected from `generate-server`, which cannot know where you will host the artifact — set `BETTER_AUTH_URL` in that server's `.env`. `start` and `deploy` write a `baseURL` themselves, so from those it means `typebase/auth.ts` sets a `baseURL` that resolves to nothing; give it a fallback or remove it.
