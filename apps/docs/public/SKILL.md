...
| `generate-server [--output ts\|esm\|cjs] [--adapter node\|bun\|hono\|fastify\|deno\|cloudflare] [--port N] [--out-dir _server] [--skip-load-env]` | Reruns `codegen`, type-checks against the fresh types, then builds a runnable server in `typebase/_server/`. Used for self-hosting or local iteration. Snapshot only — re-run after every `typebase/` change. Re-runs replace the output dir (keeping `.env` and `node_modules`); a non-empty dir not generated by Typebase is refused. Emits `src/env.ts` (a validating module compiled from your `defineEnv` schema, not a copy of `env.ts`) whenever the project has `env.ts`, `db/schema.ts`, or `auth.ts`.                                                                                                                                                                                                                                                                                                                                                                                 |
