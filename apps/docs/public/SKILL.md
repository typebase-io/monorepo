---
name: typebase
description: Write, deploy, and consume a Typebase backend — TypeScript actions, Drizzle schema, and better-auth — from inside a `typebase/` directory in an existing JS/TS project. Use when the project depends on `typebase-io` or `typebase-io-cli`, when a `typebase/` directory exists, when a `typebase.json` is present, or when the user mentions Typebase, the `action` builder, `defineAuth`, `defineEnv`, `db/schema.ts`, `db/relations.ts`, or running `typebase-io-cli` (init, codegen, deploy, db push, db pull, auth generate, env, logs, generate-server, config).
---
...
| Command                                                                                                                                   | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ----------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `init [-f] [--with-auth] [--skip-example]`                                                                                                | Scaffold the `typebase/` directory, plus a `typebase.json` at the project root holding nothing but a `$schema` reference (so editors autocomplete it while every option stays on its default). Values already in an existing `typebase.json` are kept. `-f` overwrites scaffolded files, including example schema, actions, and `env.ts`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `db pull [--url <conn>] [-f]`                                                                                                             | The other direction: read an existing PostgreSQL database and **overwrite** `db/schema.ts` and `db/relations.ts` with it, then rerun codegen. Prompts for the connection string when `--url` is omitted, and asks before replacing existing files unless `-f`. Only the `public` schema is read, but tables your tables reference in other schemas (Supabase's `auth.users`) come across as a `p.pgSchema(...)` block you have to clean up. Registers every table in relations, including the ones with no foreign key. Needs an initialized project (`init`). This is how you start from a database you already have, not something to run in a normal workflow.                                                                                                                                                                                                                               |
...
| `generate-server [--output ts\|esm\|cjs] [--adapter node\|bun\|hono\|fastify\|deno\|cloudflare] [--port N] [--out-dir _server] [--watch]` | Reruns `codegen`, type-checks against the fresh types, then builds a runnable server in `typebase/_server/`. Used for self-hosting or local iteration. Snapshot only — re-run after every `typebase/` change, or pass `--watch` to rebuild automatically (see below). Re-runs replace the output dir (keeping `.env` and `node_modules`); a non-empty dir not generated by Typebase is refused. Emits `src/env.ts` (a validating module compiled from your `defineEnv` schema, not a copy of `env.ts`) whenever the project has `env.ts`, `db/schema.ts`, or `auth.ts`. `dotenv` is added and imported only when that env module exists and the adapter is not `cloudflare` (which reads Worker bindings instead of `process.env`).                                                                                                                                                             |
...
| `config`                                                                                                                                  | Print the `typebase.json` JSON Schema on stdout with every property annotated with `x-current-value` (the value in effect, `null` when unset) and `x-is-default` (whether it came from a default); only leaf options are annotated, and `$schema` plus every `additionalProperties` are stripped, so it describes current state rather than being a validation artifact. Read-only, and the one command that does not need `typebase-io` installed. Exists so tooling can read a project's setup without reimplementing the schema or its defaults. Exits `1` with the reason on stderr if the file is malformed.                                                                                                                                                                                                                                                                               |

CLI defaults live in `typebase.json` at the project root. `init` creates it with just a `$schema` reference, so a fresh project has no option set and everything resolves to its default. Common fields: `projectPath` (defaults to `src/typebase` if a `src/` exists, else `typebase`), `serverProvider`, `server.{output,adapter,port,outDir}`, plus per-provider blocks (`vercel`, `cloudflare`, `deno`, `neon`) that the CLI fills in automatically after first deploy.
...
2. **Run the server on localhost.** `npx typebase-io-cli generate-server` writes `typebase/_server/`, then `cd typebase/_server && npm install && npx tsx src/index.ts` (default port 8080). Re-run `generate-server` and restart after **any** change in `typebase/` — each run is a snapshot; `--watch` re-runs it for you, but the server process still needs restarting. Re-runs replace `typebase/_server/` but keep `.env` and `node_modules`, so the install and env setup survive. Copy `DATABASE_URL_DEV` from project `.env` into `typebase/_server/.env` as `DATABASE_URL`, plus every key declared in `env.ts` (the server exits at boot listing whatever is missing). Add localhost origins to `trustedOrigins`. Point the frontend `TYPEBASE_APP_URL_DEV=http://localhost:8080`.

### Watch mode (`generate-server --watch`)

Builds once, then rebuilds on every change inside `typebase/`. Stops on `x`/`X`/Ctrl+C, exactly like `logs`. `_generated/` and the output directory are excluded, so a build never retriggers itself. Saving during a build cancels it and starts a fresh one, except for the final copy into the output directory, which always completes. A failed build (usually a type error) is printed and the watcher keeps running. Only the first build narrates each step; rebuilds print a single `Regenerating...` line, so "no step output" is normal rather than a sign nothing happened. The rebuilt files do not restart a server you already started from them.

**Long-running — never call it in a one-shot agent run.** Run it in the background or under a timeout, and stop it with SIGINT when the output is piped, since `x` only works on a TTY.
