...
| ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `init [-f] [--with-auth] [--with-db-publisher] [--skip-example]` | Scaffold only after inspecting every target. A partial backend can be overwritten without `-f`; never run it over existing targets without explicit authorization.                              |
| `codegen`                                                        | Refresh generated router/context types according to the rules above.                                                                                                                            |
| `auth generate`                                                  | Generate auth schema/relations/secret/types; it requires auth and DB schema files.                                                                                                              |
| `db dev push` / `db prod push`                                   | Push schema to the separate Neon branch. Stop on destructive confirmation.                                                                                                                      |
| `db local push [--url <conn>]`                                   | Push to any directly connected PostgreSQL; falls back to `DATABASE_URL`.                                                                                                                        |
| `db pull [--url <conn>] [-f]`                                    | Destructively replaces local schema and relations from the DB, then codegens. It reads `public`; cross-schema references may need cleanup. Warn first and avoid `-f` without explicit approval. |
