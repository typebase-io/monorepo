...
describe('generate-server command', () => {
  ...
  describe('reads options from typebase.json', () => {
    ...
    it('reports an absolute path when the output dir resolves to the working directory', async () => {
      await setupProject({ withAuth: true, withDb: true });
      await withCwd(tmp.path, () => generateServer.parseAsync(['--out-dir', '..'], { from: 'user' }));

      expect(tmp.exists('package.json')).toBe(true);
      expect(tmp.exists('src/index.ts')).toBe(true);
    });
  });
  ...
  describe('regeneration after the source changes', () => {
    ...
    it('leaves previously generated files in place when a source file is removed (output dir is merged, not cleaned)', async () => {
      await setupProject({ withAuth: true, withDb: true });
...
`;
  ...
  describe('failures', () => {
    ...
    it('removes a previously generated output directory when a generator fails mid-run', async () => {
      await setupProject({ withAuth: true, withDb: true });
      await withCwd(tmp.path, () => generateServer.parseAsync([], { from: 'user' }));
